<template>
	<VeeForm class="bg-green-darken-4 pa-7 rounded" :initial-values="sensor" :validation-schema #="{ meta }" @submit="update">
		<h3>Update Sensor</h3>
...
import { SensorUpdateSchema, type SensorUpdate } from "~~/shared/schema/sensor"
...
// --- Data Binding
const emit = defineEmits<{ submit: [sensor: SensorUpdate] }>()
const props = defineProps<{ sensor: SensorUpdate; error?: string; loading?: boolean }>()

// --- Pass Invoke
const update = (values: any) => emit("submit", values as SensorUpdate)
