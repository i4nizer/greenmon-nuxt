		...
		<v-row>
			<v-col class="text-end">
				...
				<v-dialog class="w-100 w-md-50" v-model="createDialog">
					...
					<template #default>
						<McuPinCreateForm :error="createError" :loading="createLoading" @submit="createPin" />
					</template>
				</v-dialog>
				<!-- Hidden Single Update Dialog -->
				<v-dialog class="w-100 w-md-50" v-model="updateDialog">
					<McuPinUpdateForm :pin="updateData" :error="updateError" :loading="updateLoading" @submit="updatePin" />
				</v-dialog>
			</v-col>
		</v-row>
		<v-row>
			<!-- Mcu Pins Table -->
			<McuPinsTable :pins="pins ?? []" @edit="editPin" @delete="deletePin" />
		</v-row>
	</v-container>
</template>
...
const { data: pins, refresh } = await useFetch<Pin[]>(
	`/api/user/greenhouse/${gid}/mcu/${mid}/pin`,
